...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, has_bias=False, weight_init=init.Normal(0.01, 0), pad_mode='pad')
...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride, has_bias=False,
                               weight_init=init.Normal(0.01, 0), pad_mode='pad')  # change
        self.bn1 = nn.BatchNorm2d(planes, affine=affine_par)
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1,  # change
                               padding=padding, has_bias=False, dilation=dilation, weight_init=init.Normal(0.01, 0),
                               pad_mode='pad')
        self.bn2 = nn.BatchNorm2d(planes, affine=affine_par)
        for i in self.bn2.get_parameters():
            i.requires_grad = False
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, has_bias=False, weight_init=init.Normal(0.01, 0),
                               pad_mode='pad')
...
class Classifier_Module(nn.Cell):
        ...
        for dilation, padding in zip(dilation_series, padding_series):
            self.conv2d_list.append(
                nn.Conv2d(inplanes, num_classes, kernel_size=3, stride=1, padding=padding, dilation=dilation,
                          has_bias=True, weight_init=init.Normal(0.01, 0), pad_mode='pad'))
    ...
    def construct(self, x):
        ...
        return out  # todo not known whether right
...
class ResNetMulti(nn.Cell):
    def __init__(self, block, layers, num_classes):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               has_bias=False, weight_init=init.Normal(0.01, 0), pad_mode='pad')
    ...
    def get_10x_lr_params(self):
        ...
        """
        b = []
        b.append(self.layer5.get_parameters())
        b.append(self.layer6.get_parameters())
...
if __name__ == '__main__':
    data = np.random.random((2, 3, 512, 1024))
